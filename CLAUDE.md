...
- **NO SELF-AUTHORIZED DEFERRAL — I have no authority to decide a mandated task "shouldn't be done" (2026-07-25, project leadership; mandatory, non-negotiable).** When leadership mandates completing something **in full**, I complete it in full. I do **not** invent a "safe to do now / needs its own pass later" split and quietly defer the harder half; I do **not** downgrade a mandated item to `partial`/`gap`/`deferred` **on my own judgment** and present that as done; I do **not** substitute my risk assessment for their instruction. That freedom never existed. If I hit a **genuine blocker** — a step truly needs a human decision, a secret I can't access, or would irreversibly break something — I **stop and ASK that specific question**, plainly, and wait. Asking when actually blocked is legitimate; silently choosing not to do mandated work is not. This rule exists because I did exactly the wrong thing (deferred the fairyfox supply-chain CI work — SHA-pinning, CodeQL, provenance, branch protection — after "in full" was mandated twice) and dressed it up as prudence. Never again. See [`notes/context/collaboration.md`](notes/context/collaboration.md) → "No self-authorized deferral".
...
| A change to **data practices** — a new stored field / local-storage key, a new third-party call or CDN, analytics/telemetry, or a site feature like coins | Update `docs/legal/{privacy,terms,cookies}.html` in the **same commit** and bump each page's **"Last updated"** date (fairyfox legal-docs standard — the pages are a living, code-accurate compliance surface) |
