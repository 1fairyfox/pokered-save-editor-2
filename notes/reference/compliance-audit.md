...
| badges | **done** | README carries the full canonical set — the 5 previously-absent build/quality/security slots (Coverage, Code quality, Quality gate, Tech debt, OpenSSF Scorecard) added 2026-08-03 now that the services are wired, alongside contributors/stars/forks/watchers/commits/CI/docs/pages/version/issues/PRs/license. *Sub-note:* docs badge → `1fairyfox.github.io/<key>/` (where docs deploy); new service badges populate on first main run (documented badge-lag). |
