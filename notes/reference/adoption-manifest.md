...
| ci-secrets | implemented | 1.6.1 / 2d614f0 | 2026-08-03 (secrets set) | **All 3 secrets set + referenced** — `gh secret list`: `SONAR_TOKEN`, `CODECOV_TOKEN`, `SCORECARD_TOKEN` (provisioned via `scripts/repo-tokens.ps1`), each referenced by a workflow (`sonar.yml`, `coverage.yml`, `scorecard.yml`); canonical names; nothing referenced-but-unset — the standard's Verify passes. ⚠️ The three workflows first **execute** on the next push/PR to `main` (GitHub only runs a workflow once it's on the default branch), so their authenticated runs go green at the next release — a confirmation, not a gap, and they're not `main`-required contexts so they can't block a release. |
...
| legal-docs | implemented | 1.6.1 / 2d614f0 | 2026-07-25 | `docs/legal/{privacy,terms,cookies}.html` — self-hosted, code-accurate (desktop app stores nothing on a server; the docs site discloses GitHub Pages processor, self-hosted fonts, and the reader-prefs + coins **local storage** with clear/reset controls); coins-not-money + `/legal/coins/` link; `legal@fairyfox.io` contact; deployed to `/legal/` by `pages.yml`; linked in the chrome footer "This project" column + a `Legal` subnav item; CLAUDE.md maintenance trigger added |
...
| badges | implemented | 1.6.1 / 2d614f0 | 2026-08-03 | `README.md` carries the full canonical set — the 5 build/quality/security slots (Coverage, Code quality, Quality gate, Tech debt, OpenSSF Scorecard) added now that the services are wired, plus contributors/stars/CI/docs/pages/version/issues/PRs/license. *Sub-note:* docs badge → `1fairyfox.github.io/<key>/` (where docs deploy); the new service badges populate on the services' first main run (documented badge-lag). |
