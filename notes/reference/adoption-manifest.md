...
| ci-secrets | partial (owner token provisioning) | 1.6.1 / 2d614f0 | 2026-07-25 | **All 3 authenticated integrations wired:** Scorecard `repo_token: ${{ secrets.SCORECARD_TOKEN \|\| github.token }}` (fuller coverage — Branch-Protection/webhooks — falling back so it stays green); `coverage.yml` → Codecov upload (`CODECOV_TOKEN`, gated); `sonar.yml` + `sonar-project.properties` → SonarCloud (`SONAR_TOKEN`, gated). **Owner step (external accounts I cannot create):** run **`scripts/repo-tokens.ps1`** — it prints each token's get-it link, prompts with hidden input, and sets all three via `gh` (values never hit disk/history/transcript). Gated so CI stays green until then. See the process report → "ci-secrets provisioning". |
